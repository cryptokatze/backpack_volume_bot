...
## 8. 메뉴 사용법 한눈에 보기
실행 후 나오는 기본 메뉴:
- 1) 잔고/포지션 확인: 실시간으로 표시, Enter를 누르면 메뉴로 돌아옵니다. (아직 구현이 안됨 TODO.)
...
옵션 4번을 선택하면 심볼(티커) → 주문 수량 → 각 방향 주문 횟수 순서대로 입력하는 설정 단계가 시작됩니다.

1️⃣ 루틴 기본 구조

예를 들어 아래와 같이 입력했다고 가정해봅시다.
...
➡️ 이 경우 실행 루틴은 다음과 같이 구성됩니다.

BTC 0.01 매수 × 3회
BTC 0.01 매도 × 3회


즉, 매수 3번 → 매도 3번이 하나의 세트(Set)입니다.

2️⃣ 세트 반복 (Loop Count)

세트 반복은 위에서 만든 **전체 루틴(매수+매도 세트)**을
몇 번 반복할지 결정합니다.
...
⚠️ 무한 반복을 사용할 때는
포지션 누적(익스포저) 관리에 반드시 주의하세요.

3️⃣ 대기시간 (Interval)

각 주문 사이에 기다릴 시간을 설정합니다.
단위는 초(second) 이며, 0.01초까지 입력할 수 있습니다.

예시:

0.05 → 0.05초 간격으로 주문 실행


⚠️ 주의
거래소 API 정책에 따라 과도한 주문 빈도는
트래픽 제한 또는 API 차단으로 이어질 수 있습니다.

분당 2,000건 이상의 주문은 절대 권장하지 않습니다.

4️⃣ 실행 방법

설정을 모두 마쳤다면 메뉴에서 2번을 선택하여
방금 입력한 구성대로 주문 루틴을 실행할 수 있습니다.
